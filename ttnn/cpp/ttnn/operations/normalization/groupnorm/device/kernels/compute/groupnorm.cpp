...
void kernel_main() {
    ...
    // block is tilized on-core into cb_in at its point of consumption (see GN_TILIZE_INPUT_BLOCK
    // below), mirroring how the sharded kernel tilizes a ROW_MAJOR shard. The block is re-read
    // (and re-tilized) once per GroupNorm pass, matching the interleaved reader's 3-pass design.
    binary_op_init_common(cb_in0_id, cb_in0_id, cb_in_id);
#else
    binary_op_init_common(cb_in0_id, cb_input_mask_id, cb_x_id);
#endif

// Tilize the current input block (block_w tiles wide, out_block_h_actual tile-rows tall) from the
// row-major cb_in0 into the tiled cb_in, then wait for the tiled block. WaitMode::WaitBlock makes
// tilize consume (wait+pop) the cb_in0 pages the reader pushed for this block. Under the non-tilize
// (TILE input) path this is a plain cb_in0.wait_front, leaving the tiled flow unchanged.
#ifdef TILIZE_IN
#define GN_TILIZE_INPUT_BLOCK()                                                                                     \
    do {                                                                                                            \
...
#define GN_INPUT_SRC_ID cb_in_id
#define GN_POP_INPUT_BLOCK(n) cb_in.pop_front(n)
#else
#define GN_TILIZE_INPUT_BLOCK() cb_in0.wait_front(out_block_hw_normal)
#define GN_INPUT_SRC_ID cb_in0_id
#define GN_POP_INPUT_BLOCK(n) cb_in0.pop_front(n)
#endif
    ...
    for (uint32_t b = 0; b < batch; ++b) {
        ...
        for (uint32_t g = 0; g < group; ++g) {
            ...
            for (uint32_t out_block_index = 0; out_block_index < num_out_blocks_padded; out_block_index++) {
                ...
                for (uint32_t i = 0; i < out_block_h_actual; ++i) {
                    ...
                    for (uint32_t j = 0; j < num_subblocks_w; ++j) {
                        ...
                        for (uint32_t w = 0; w < subblock_w; ++w) {
                            ...
                            mul_tiles(GN_INPUT_SRC_ID, cb_input_mask_id, index, index_mask, w);
                        }
                        tile_regs_commit();
                        tile_regs_wait();
                ...
                GN_POP_INPUT_BLOCK(out_block_hw_normal);
                ...
                for (uint32_t i = 0; i < out_block_h_actual; i++) {
                    ...
                    for (uint32_t j = 0; j < num_subblocks_w; j++) {
                        ...
                        for (uint32_t w = 0; w < subblock_w; w++) {
                            ...
                            sub_tiles_bcast_scalar(GN_INPUT_SRC_ID, cb_ex_global_id, index, 0, w);
                        }
                        tile_regs_commit();
                        tile_regs_wait();
                        for (uint32_t i = 0; i < subblock_w; i++) {
                    ...
                    GN_POP_INPUT_BLOCK(block_w);
                }
                if (extra_out_block && (out_block_index == (num_out_blocks_padded - 1))) {
                    GN_POP_INPUT_BLOCK(out_block_hw_normal - out_block_hw_last);
                }
