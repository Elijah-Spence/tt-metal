...
GatherDeviceOperation::program_factory_t GatherDeviceOperation::select_program_factory(
    ...
    if (tensor_args.input_tensor.layout() == Layout::ROW_MAJOR) {
        // RM mirrors TILE's width-based split: when index row is wide, parallelize across
        // columns; otherwise parallelize across rows.
        const uint32_t W_input = input_tensor_shape[-1];
        ...
        if (W_input > rm_w_threshold || W_index > rm_w_threshold) {
