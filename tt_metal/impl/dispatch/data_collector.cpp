...
void DataCollector::UnregisterProgramRealtimeProfilerCallback(tt::ProgramRealtimeProfilerCallbackHandle handle) {
    ...
    // Notify under the lock, like Register/Attach: DetachRealtimeProfilerCallbackListener takes the
    // same mutex, so holding it here prevents a listener from being removed and destroyed while a
    // callout into it is in flight.
    for (auto* listener : realtime_callback_listeners_) {
...
void DataCollector::DetachRealtimeProfilerCallbackListener(tt::RealtimeProfilerCallbackListener* listener) {
    std::lock_guard<std::mutex> lock(program_realtime_profiler_callbacks_mutex_);
    auto it = std::find(realtime_callback_listeners_.begin(), realtime_callback_listeners_.end(), listener);
    if (it != realtime_callback_listeners_.end()) {
        realtime_callback_listeners_.erase(it);
    }
}
