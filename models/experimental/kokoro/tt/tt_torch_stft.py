...
The OLA conv_transpose minimises its DRAM height-slice count: the sliced conv over-computes per
slice (each slice is a heavily-padded conv + its own Halo/PaddedSlice/SliceWrite), so fewer/bigger
slices are far faster — collapsing F~5k from 4 slices to 1, dropping the pow2 rounding, and folding
the trim cut the standalone iSTFT device time 1957->~266µs.  But per-slice L1 footprint grows with
frames/slice while the L1 left for this conv SHRINKS as the audio lengthens (the rest of the
generator is resident).  So ``_ct_ola_run`` picks the slice count from two deterministic
regimes: short audio (F <= 9728) uses ONE slice (fast); longer audio uses ~4096 frames/slice, which
is measured to fit even at max L1 pressure (demo F=49321 -> 13 slices, max-length F~138k -> 34
slices).  ``act_block_h_override=32`` shrinks the conv's L1 CBs so short-to-medium audio fits in a
single slice inside the full generator.
...
# iSTFT conv_transpose2d OLA DRAM-height-slice sizing (frames per slice).  The frames/slice that
# fits L1 shrinks as the audio lengthens (more of the generator stays resident), so two regimes:
#   * F <= _CT_SINGLE_SLICE_MAX_FRAMES: ONE slice — the fast path (fits the full generator).
#   * longer audio: _CT_SAFE_FRAMES_PER_SLICE frames/slice — fits even at max L1 pressure.
# _CT_ACT_BLOCK_H caps the conv's per-core activation circular buffer so a single slice fits L1.
_CT_SINGLE_SLICE_MAX_FRAMES = 9728
_CT_SAFE_FRAMES_PER_SLICE = 4096
_CT_ACT_BLOCK_H = 32
...
class TTTorchSTFT:
    ...
    def _ct_ola_run(self, x_nhwc: ttnn.Tensor, n_frames: int, pad: int) -> ttnn.Tensor:
        ...
        # DRAM height slices: the sliced conv_transpose over-computes per slice (each slice runs a
        # heavily padded conv plus its own Halo/PaddedSlice/SliceWrite), so FEWER, BIGGER slices are
        # far faster — at F~5k standalone, 1 slice runs the OLA in ~64µs where the old pow2-rounded
        # 4-slice path cost ~1200µs.  But the per-slice L1 footprint grows with frames/slice, and the
        # L1 left for this conv SHRINKS as the audio lengthens (the rest of the generator — decoder,
        # upsample convs — is resident and scales with audio length).  So the frames-per-slice that
        # fits L1 DECREASES with F, and no single ``ceil(F / const)`` tracks it.  Two deterministic
        # regimes (measured, no trial-and-error — see the OOM ceilings below):
        #   * short audio (F <= 9728): ONE slice — the fast path.  Verified to fit inside the full
        #     generator up to F=9721 (with act_block_h_override above).
        #   * longer audio: 4096 frames/slice.  Fits even at max L1 pressure — verified demo F=49321
        #     (13 slices) and max-length F~138k (34 slices); the measured OOM ceiling is ~9864
        #     frames/slice at F=49321 and ~5760 at F~138k, so 4096 clears it with margin.
        # The ceil is DIRECT (not pow2-rounded — the BH conv2d DRAM-slice factory accepts any count,
        # cf. tt_custom_stft using 1 and 2).
        ceil_safe = (n_frames + _CT_SAFE_FRAMES_PER_SLICE - 1) // _CT_SAFE_FRAMES_PER_SLICE
        num_slices = 1 if n_frames <= _CT_SINGLE_SLICE_MAX_FRAMES else ceil_safe
